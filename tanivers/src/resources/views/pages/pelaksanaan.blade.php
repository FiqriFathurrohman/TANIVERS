@php
    ...
    if (!isset($tasksHariIni) || $tasksHariIni->isEmpty()) {
        ...
        foreach ($tasksHariIni as $task) { $task->is_fallback = false; }
    }
@endphp
            ...
            <div class="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm space-y-4">
                ...
                <div class="task-list space-y-2 text-xs">
                    @foreach($tasksHariIni as $task)
                        @php $isDone = ($task->id && in_array($task->id, $checkedTasks)); @endphp
                        <div class="flex items-center justify-between p-4 rounded-2xl border select-none {{ $isDone ? 'bg-green-50/80 border-green-200' : 'bg-white border-slate-200' }}"
                             @if(!($task->is_fallback ?? false) && $task->id) onclick="submitAjaxToggle({{ $task->id }}, {{ $currentHst }}, this)" @endif
                             style="{{ ($task->is_fallback ?? false) ? 'cursor: default;' : 'cursor: pointer;' }}">
                            <div class="flex items-center gap-3">
                                <div class="w-5 h-5 rounded-md border-2 flex items-center justify-center font-bold text-[10px] checkbox-box {{ $isDone ? 'bg-green-600 border-green-600 text-white' : 'border-slate-300 text-transparent' }}">✓</div>
                                <div>
            ...
            <div class="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm space-y-4">
                ...
                <form id="form-ajax-photo-log" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-12 gap-3 bg-slate-50 p-4 rounded-2xl border">
                    ...
                    <div class="md:col-span-8 flex flex-col justify-between space-y-2">
                        ...
                        <button type="submit" id="btn-submit-foto-ajax" class="w-full bg-emerald-700 hover:bg-emerald-800 text-white font-bold p-2 rounded-xl text-xs shadow-sm transition-all">Simpan Bukti Foto Log</button>
                    </div>
                </form>
                    ...
                    @forelse($photoLogs as $p)
                        <div class="bg-slate-50 border rounded-xl overflow-hidden relative group cursor-pointer hover:shadow-md transition-all border-slate-200"
                            ...
                            <div class="p-2 text-[10px] flex flex-col justify-between h-auto">
                                <div><div class="flex justify-between font-bold text-slate-700"><span>{{ $p->fase_tanaman ?? 'Budidaya' }}</span><span class="text-green-700">{{ $p->current_hst ?? $currentHst }} HST</span></div><p class="text-[9px] text-slate-400 truncate mt-0.5">{{ $p->keterangan }}</p></div>
                                <span class="text-[9px] font-mono text-slate-400 block mt-2 border-t pt-1 border-slate-200/60">📅 {{ \Carbon\Carbon::parse($p->created_at)->translatedFormat('d M Y') }}</span>
                            </div>
                        </div>
                    @empty
                        <p class="text-slate-400 italic text-center py-4 col-span-3 text-xs">Belum ada dokumentasi visual tanaman di database.</p>
                    @endforelse
                </div>
            </div>
        </div>
            ...
            <div class="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm space-y-4">
                ...
                <form id="form-kas-keluar" action="{{ route('pelaksanaan.keuangan.store') }}" method="POST" class="space-y-3 text-xs">
                    ...
                    <div id="wrapper-manajemen-buruh" class="grid grid-cols-2 gap-3 p-3 bg-green-50/50 border border-green-100 rounded-xl hidden">
                        <div><label class="font-bold text-slate-500 block mb-1">Jumlah Buruh (Orang)</label><input type="number" name="jumlah_buruh" id="input-jumlah-buruh" min="0" class="w-full p-2.5 border bg-white rounded-xl outline-none" placeholder="0" value="0"></div>
                        <div><label class="font-bold text-slate-500 block mb-1">Upah / Orang (Rp)</label><input type="number" name="upah_per_orang" id="input-upah-buruh" min="0" class="w-full p-2.5 border bg-white rounded-xl outline-none" placeholder="Contoh: 80000" value="0"></div>
                    </div>
                    <div id="wrapper-nominal-manual"><label class="font-bold text-slate-500 block mb-1">Nominal Biaya (Rp)</label><input type="number" name="nominal" id="input-nominal-manual" min="0" class="w-full p-3 border border-slate-200 rounded-xl outline-none focus:border-green-600" placeholder="Contoh: 150000"></div>
                    <div><label class="font-bold text-slate-500 block mb-1">Keterangan Nota Lapangan</label><textarea name="keterangan" id="textarea-keterangan" rows="2" required class="w-full p-3 border border-slate-200 rounded-xl outline-none focus:border-green-600" placeholder="Contoh: Beli Pupuk Urea tambahan petak barat"></textarea></div>
                    <button type="submit" class="w-full bg-yellow-500 hover:bg-yellow-600 text-slate-900 font-extrabold p-3.5 rounded-xl shadow-md transition-all">💾 Simpan Catatan Pengeluaran Riil</button>
                </form>
            </div>
                            ...
                            @if(isset($logKeuangan) && $logKeuangan->count() > 0)
                                @foreach($logKeuangan as $log)
                                    <tr class="text-slate-600 border-b">
                                        ...
                                        <td class="p-3 font-mono text-[10px] text-slate-400">{{ \Carbon\Carbon::parse($log->tanggal_input)->translatedFormat('d M Y') }}</td>
                                    </tr>
                                @endforeach
                            @else
                                <tr><td colspan="4" class="text-center py-6 text-xs text-slate-400 italic">Belum ada catatan kas keluar musim ini di database.</td></tr>
                            @endif
                        </tbody>
                    </table>
                </div>
...
<div id="modal-detail-foto" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center hidden opacity-0 transition-opacity duration-300" onclick="tutupModalFoto()">
    <div class="bg-white w-full max-w-lg p-5 rounded-[2rem] border border-slate-200 shadow-2xl space-y-4 m-4 transform scale-95 transition-transform duration-300" onclick="event.stopPropagation()">
        <div class="flex justify-between items-center border-b pb-2"><h4 class="text-sm font-black text-slate-800 uppercase tracking-wider">📄 Detail Log Foto Tanaman</h4><button class="text-slate-400 hover:text-slate-600 font-bold text-lg" onclick="tutupModalFoto()">✕</button></div>
        <div class="w-full h-64 bg-slate-100 rounded-2xl overflow-hidden border"><img id="modal-img-target" src="" class="w-full h-full object-cover"></div>
        <div class="space-y-2 text-xs text-slate-600">
            <div class="grid grid-cols-2 gap-2 bg-slate-50 p-3 rounded-xl border border-slate-100"><div>🌾 <span class="text-slate-400">Kategori:</span> <strong id="modal-kategori-target"></strong></div><div>📅 <span class="text-slate-400">Umur Padi:</span> <strong id="modal-hst-target"></strong></div></div>
            <div class="p-3 bg-emerald-50/40 rounded-xl border border-emerald-100"><span class="text-slate-400 block font-semibold mb-0.5">📝 Catatan Keterangan Lapangan:</span><p id="modal-keterangan-target" class="text-slate-800 font-medium leading-relaxed"></p></div>
        </div>
        <button class="w-full bg-slate-800 hover:bg-slate-900 text-white font-bold p-3 rounded-xl text-xs transition-all shadow-md" onclick="tutupModalFoto()">Close Detail Preview</button>
    </div>
</div>
...
<script>
    function bukaModalFoto(src, kategori, hst, keterangan) {
        ...
        setTimeout(() => { modal.classList.remove('opacity-0'); modalBox.classList.remove('scale-95'); modalBox.classList.add('scale-100'); }, 20);
    }
    ...
    function submitAjaxToggle(id, hst, element) {
        ...
        if (isCompletedNow) {
            element.classList.add('bg-green-50/80', 'border-green-200'); element.classList.remove('bg-white', 'border-slate-200');
            const checkbox = element.querySelector('.checkbox-box'); if (checkbox) checkbox.classList.add('bg-green-600', 'border-green-600', 'text-white');
            const badge = element.querySelector('.badge-status'); if (badge) { badge.className = "badge-status text-[9px] font-bold px-2 py-0.5 rounded bg-green-100 text-green-800"; badge.innerText = 'Selesai'; }
        } else {
            element.classList.add('bg-white', 'border-slate-200'); element.classList.remove('bg-green-50/80', 'border-green-200');
            const checkbox = element.querySelector('.checkbox-box'); if (checkbox) checkbox.classList.remove('bg-green-600', 'border-green-600', 'text-white');
            const badge = element.querySelector('.badge-status'); if (badge) { badge.className = "badge-status text-[9px] font-bold px-2 py-0.5 rounded bg-amber-100 text-amber-800"; badge.innerText = 'Belum Selesai'; }
        }
        fetch("{{ route('sop.toggle') }}", { method: "POST", headers: { "Content-Type": "application/json", "X-CSRF-TOKEN": "{{ csrf_token() }}" }, body: JSON.stringify({ sop_template_id: id, hst: hst, completed: isCompletedNow }) })
        .then(res => res.json()).then(data => { if (!data.success) { console.error("Gagal menyimpan checklist:", data.message); alert("Gagal: " + data.message); location.reload(); } })
        .catch(err => { console.error("Error:", err); alert("Terjadi kesalahan jaringan."); });
    }
    document.getElementById('input-foto-log').addEventListener('change', function() { document.getElementById('nama-file-pilihan').innerText = this.files[0] ? this.files[0].name : ''; });
    document.getElementById('form-ajax-photo-log').addEventListener('submit', function(e) {
        e.preventDefault(); const btn = document.getElementById('btn-submit-foto-ajax'); const originalText = btn.innerText; btn.innerText = "⏳ Mengunggah..."; btn.disabled = true;
        let formData = new FormData(this);
        fetch("{{ route('pelaksanaan.photo.store') }}", { method: "POST", body: formData, headers: { "X-Requested-With": "XMLHttpRequest" } })
        .then(res => res.json()).then(data => { if (data.success) { btn.innerText = "✓ Berhasil!"; setTimeout(() => location.reload(), 1000); } else { alert("Gagal: " + data.message); btn.innerText = originalText; btn.disabled = false; } })
        .catch(err => { console.error(err); alert("Error: " + err.message); btn.innerText = originalText; btn.disabled = false; });
    });
    document.getElementById('form-kas-keluar').addEventListener('submit', function(e) {
        e.preventDefault(); const form = this; const btn = form.querySelector('button[type="submit"]'); const originalText = btn.innerText; btn.innerText = "⏳ Menyimpan..."; btn.disabled = true;
        let formData = new FormData(form);
        fetch(form.action, { method: "POST", body: formData, headers: { "X-Requested-With": "XMLHttpRequest" } })
        .then(res => res.json()).then(data => { if (data.status === 'success') { btn.innerText = "✓ Tersimpan!"; setTimeout(() => location.reload(), 1000); } else { alert("Gagal: " + data.message); btn.innerText = originalText; btn.disabled = false; } })
        .catch(err => { console.error("Gagal simpan:", err); alert("Terjadi kesalahan: " + err.message); btn.innerText = originalText; btn.disabled = false; });
    });
</script>