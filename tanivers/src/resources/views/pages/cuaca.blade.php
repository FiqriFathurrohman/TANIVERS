@php
    // 🎯 AMBIL DATA LAHAN SECARA AMAN DARI LOGIN SIKLUS USER ALFIN
    ...
    // 🎯 STRATEGI PARSING SAKRAL: Ambil kata pertama saja (Misal: 'Inpara Series' diambil 'Inpara')
    // Ini menjamin kecocokan 100% dengan kolom variety_group di seeder lu Fin!
...
<div id="page-cuaca" class="page-section hidden space-y-6 w-full">
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 pb-4">
        <div>
          <h3 class="text-lg sm:text-xl font-black text-slate-800 uppercase tracking-tight flex items-center gap-2">
              📡 Radar Satelit Cuaca & Geolokasi Multi-Device
          </h3>
        ...
        <div id="gps-badge" class="bg-amber-50 border border-amber-200 text-amber-700 text-[10px] sm:text-[11px] font-black px-3 py-2 rounded-xl flex items-center gap-1.5 animate-pulse shrink-0">
            ⏳ Meminta Akses GPS Perangkat...
        </div>
    </div>
    ...
    <div class="bg-gradient-to-br from-emerald-800 to-emerald-950 p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] text-white shadow-xl relative overflow-hidden border border-emerald-900 group transition-all">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-center relative z-10">
            <div class="lg:col-span-7 flex flex-col sm:flex-row items-center sm:items-start lg:items-center gap-4 sm:gap-6 text-center sm:text-left">
                ...
                <div class="w-full min-w-0">
                    <div class="flex items-baseline justify-center sm:justify-start gap-1">
                        <span id="satelit-main-temp" class="text-5xl sm:text-6xl md:text-7xl font-black tracking-tighter text-yellow-400">--°C</span>
                    </div>
            ...
            <div class="lg:col-span-5 grid grid-cols-2 gap-3 w-full text-left text-xs">
                <div class="bg-emerald-900/40 border border-emerald-700/30 p-4 rounded-2xl">
                    <span class="text-emerald-300 font-bold block text-[11px] uppercase">💧 Kelembaban</span>
                    <span id="satelit-humidity" class="text-lg sm:text-xl font-black text-white">-- %</span>
                    <p class="text-[9px] sm:text-[10px] text-emerald-200/60 mt-0.5">Kadar uap air mikro</p>
                </div>
                <div class="bg-emerald-900/40 border border-emerald-700/30 p-4 rounded-2xl">
                    <span class="text-emerald-300 font-bold block text-[11px] uppercase">💨 Kec. Angin</span>
                    <span id="satelit-wind" class="text-lg sm:text-xl font-black text-white">-- km/h</span>
                    <p class="text-[9px] sm:text-[10px] text-emerald-200/60 mt-0.5">Hembusan udara lokal</p>
                </div>
                <div class="bg-emerald-900/40 border border-emerald-700/30 p-4 rounded-2xl">
                    <span class="text-emerald-300 font-bold block text-[11px] uppercase">🌦️ Indeks UV</span>
                    <span id="satelit-uv" class="text-lg sm:text-xl font-black text-white">--</span>
                    <p class="text-[9px] sm:text-[10px] text-emerald-200/60 mt-0.5">Radiasi sinar matahari</p>
                </div>
                <div class="bg-emerald-900/40 border border-emerald-700/30 p-4 rounded-2xl">
                    <span class="text-emerald-300 font-bold block text-[11px] uppercase">☁️ Tutupan Awan</span>
                    <span id="satelit-cloud" class="text-lg sm:text-xl font-black text-white">-- %</span>
                    <p class="text-[9px] sm:text-[10px] text-emerald-200/60 mt-0.5">Densitas mendung</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white p-5 sm:p-6 rounded-[2rem] border border-slate-200 shadow-sm space-y-4">
        ...
        <div id="satelit-forecast-container" class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 w-full">
            <div class="col-span-full text-center py-6 text-xs font-bold text-slate-400">Sedang mengunduh data prakiraan dari satelit cuaca...</div>
        </div>
    </div>
    ...
    <div class="bg-white p-5 sm:p-6 rounded-[2rem] border border-slate-200 shadow-sm space-y-4 text-left">
        <div class="flex justify-between items-center border-b border-slate-100 pb-2">
            <div>
                <h4 class="text-xs font-black text-slate-400 uppercase tracking-wider">🚨 NOTIFIKASI DETEKSI HAMA BERDASARKAN BULAN & REKOMENDASI VARIETAS RIIL</h4>
                <p class="text-[11px] text-slate-400 mt-0.5">Kecocokan mitigasi agronomis untuk varietas <strong class="text-emerald-700">{{ $varietasUser }}</strong> pada <strong class="text-emerald-700">HST {{ $hstUser }}</strong>.</p>
            </div>
                ...
                <div class="p-4 bg-slate-50 border border-slate-200 rounded-2xl shadow-sm flex gap-3 items-start group hover:scale-[1.01] transition-all">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex flex-wrap items-center justify-between gap-1">
                            <strong class="font-black text-slate-800 text-sm">{{ $hama->nama_hama }}</strong>
                            <span class="text-[9px] font-black uppercase tracking-wider text-red-600 bg-red-50 px-2 py-0.5 rounded border border-red-100">{{ $hama->status_alert }}</span>
                        </div>
                        <p class="text-[11px] text-slate-500 mt-1 leading-relaxed text-left">{{ $hama->deskripsi_mitigasi }}</p>
                    </div>
                </div>
            @empty
                <div class="col-span-full text-center py-6 bg-emerald-50 text-emerald-800 font-bold border border-emerald-200 rounded-2xl shadow-sm">
                    ✓ Ekosistem Terjaga Aman: Berdasarkan seeder database untuk umur {{ $hstUser }} HST varietas {{ $varietasUser }}, kondisi ketahanan tanaman padi Anda dalam kondisi aman seimbang dari hama makro.
                </div>
            @endforelse
        </div>
    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if(badgeElement) {
            badgeElement.innerText = `${listBulan[skrg.getMonth()]} ${skrg.getFullYear()}`;
        }
    });

    function initRadarCuacaSatelit() {
        if (!navigator.geolocation) {
            updateGpsBadge("❌ GPS Not Supported", "bg-red-100 text-red-700 border-red-200");
            loadCuacaDefault();
            return;
        }
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
                console.warn("Akses lokasi dibatasi perangkat, memuat default lokasi Alfin:", error.message);
                loadCuacaDefault();
            },
    ...
    function loadCuacaDefault() {
        ...
        const defaultLat = -6.2886;
        const defaultLon = 106.7179;
    ...
    function updateGpsBadge(text, classes) {
        ...
        if(badge) {
            ...
            badge.className = `${classes} text-[10px] sm:text-[11px] font-black px-3 py-1.5 rounded-xl flex items-center gap-1.5 shrink-0`;
        }
    }
        ...
        fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lon}`)
            ...
            .then(data => {
                if (data && data.display_name) {
                    document.getElementById('satelit-address').innerText = data.display_name;
                }
            });
    }
    ...
    function renderForecastMingguan(daily) {
        ...
        daily.time.forEach((time, index) => {
            ...
            container.innerHTML += `
                <div class="${bgBox} p-3.5 rounded-2xl text-center border shadow-sm flex flex-col justify-between items-center hover:bg-white hover:shadow-md transition-all">
                    <span class="text-[10px] font-black ${index === 0 ? 'text-emerald-800' : 'text-slate-400'} block uppercase tracking-wider">${dayName}</span>
                    <span class="text-2xl block my-2">${weather.icon}</span>
                    <div>
                        <span class="text-xs font-black text-slate-800 block">${maxTemp}°C</span>
                        <span class="text-[9px] text-slate-400 font-medium block mt-0.5">${minTemp}°C</span>
                    </div>
                </div>
            `;
        });
