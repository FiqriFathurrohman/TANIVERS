...
<head>
    ...
    <style>
        ...
        ::-webkit-scrollbar-thumb { background: #A7F3D0; border-radius: 20px; }
        ::-webkit-scrollbar-thumb:hover { background: #059669; }

        /* Efek Premium Sidebar Hijau Tua */
        .sidebar-premium {
    ...
    <aside class="w-72 sidebar-premium border-r border-emerald-900/50 p-6 hidden md:flex flex-col justify-between shrink-0 h-full shadow-[4px_0_24px_rgba(0,0,0,0.1)] text-white">
        ...
        <div class="relative z-10 flex flex-col h-full">
            <div class="flex items-center gap-3.5 mb-10 shrink-0">
                ...
                <div>
                    <span class="font-extrabold text-white text-xl tracking-tight block">Tanivers</span>
                    <span class="text-[10px] text-emerald-300 font-medium tracking-widest uppercase block mt-0.5">Sistem Pintar</span>
                </div>
            </div>
            
            <nav class="space-y-2 flex-1 overflow-y-auto pr-2">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3.5 px-4 py-3.5 rounded-xl transition-all {{ Route::is('dashboard') ? 'bg-gradient-to-r from-emerald-500 to-emerald-600 text-white shadow-md border border-emerald-400/30' : 'text-emerald-100/70 hover:bg-emerald-800/40 hover:text-white' }} text-sm font-medium">
                    <i data-lucide="layout-dashboard" class="w-5 h-5"></i>
                    Dashboard
                </a>

                <a href="{{ route('lahan.create') }}" class="flex items-center gap-3.5 px-4 py-3.5 rounded-xl transition-all {{ Route::is('lahan.create') ? 'bg-gradient-to-r from-emerald-500 to-emerald-600 text-white shadow-md border border-emerald-400/30' : 'text-emerald-100/70 hover:bg-emerald-800/40 hover:text-white' }} text-sm font-medium">
                    <i data-lucide="map" class="w-5 h-5"></i>
                    Daftarkan Lahan
                </a>

                <a href="#" class="flex items-center gap-3.5 px-4 py-3.5 rounded-xl transition-all text-emerald-100/70 hover:bg-emerald-800/40 hover:text-white text-sm font-medium">
        ...
        <div class="pt-6 mt-4 border-t border-emerald-800/50 flex flex-col gap-4 relative z-10 shrink-0">
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                    <p class="text-sm font-bold text-white truncate">{{ Auth::user()->name ?? 'Petani Tanivers' }}</p>
                    <p class="text-[11px] text-emerald-300/80 mt-0.5 truncate">{{ Auth::user()->district ?? 'Tangerang' }}</p>
                </div>
            </div>
