...
{ 
    ...
    { 
        ...
        // --- GEMBOK 2: FILTER MASA TANAM KADALUARSA ---
        $plans = PreProductionPlan::with([ 
            'lahan', 'commodity', 'commodityType', 'plantingGuide.phases.tasks', 
        ]) 
        ->where('user_id', Auth::id()) 
        ->where('is_active', true) 
        // Ini kuncinya: Hanya tampilkan kalau current_day belum melewati duration_days
        ->whereColumn('current_day', '<=', 'duration_days') 
        ->latest() 
        ->get(); 
        // --- END GEMBOK 2 ---
        ...
        if ($selectedPlan && $selectedPlan->plantingGuide) { 
            ...
            $currentPhase = $selectedPlan->plantingGuide->phases 
                ->first(function ($phase) use ($currentDay) { 
                    return $currentDay >= (int) $phase->start_day && $currentDay <= (int) $phase->end_day; 
                }); 
        ...
        return view('pelaksanaan.index', compact( 
            'plans', 'selectedPlan', 'currentPhase', 'todayTasks', 'checkedTaskIds', 
            'pests', 'diseases', 'pestReports', 'expenseReports', 'expenseTotal' 
        )); 
    } 
    ...
    { 
        ...
        $hasPestReport = $request->filled('pest_id') || $request->filled('disease_id') || $request->hasFile('photo') || $request->filled('pest_notes'); 
        ...
        if ($hasPestReport) { 
            ...
            if ($request->hasFile('photo')) { 
                $folder = $reportType === 'penyakit' ? 'penyakit-reports' : 'hama-reports'; 
        ...
        if ($expenseItems->isNotEmpty()) { 
            ...
            // --- ALERT KEUANGAN REAL-TIME --- 
            $budgetTarget = (float) $plan->budget; 
            if ($budgetTarget > 0) { 
                $totalSpent = (float) ExecutionExpense::where('pre_production_plan_id', $plan->id)->sum('total_amount'); 
                $percentage = ($totalSpent / $budgetTarget) * 100; 
                
                if ($percentage >= 80) { 
                    $plan->loadMissing(['user', 'lahan', 'commodityType']); 
                    Mail::to(Auth::user()->email)->send(new FinancialAlertMail($plan, $totalSpent, $percentage)); 
                } 
            } 
            // --- END ALERT --- 
        } 
    ...
    { 
        ...
        // Update hari di database 
        $plan->update([ 
            'current_day' => (int) $validated['current_day'], 
        ]); 

        // --- SENSOR PANEN REAL-TIME (H-7) --- 
        $daysLeft = (int) $plan->duration_days - (int) $plan->current_day; 
        if ($daysLeft === 7) { 
            $plan->loadMissing(['user', 'lahan', 'commodityType']); 
            Mail::to(Auth::user()->email)->send(new HarvestReminderMail($plan)); 
        } 
        // --- END SENSOR PANEN --- 
    ...
    { 
        ...
        $photoPath = null; 
        if ($request->hasFile('photo')) { 
            $folder = $validated['report_type'] === 'penyakit' ? 'penyakit-reports' : 'hama-reports'; 
            $photoPath = $request->file('photo')->store($folder, 'public'); 
        } 
        ...
        $message = $validated['report_type'] === 'penyakit' ? 'Laporan penyakit berhasil dikirim.' : 'Laporan hama berhasil dikirim.'; 
        return redirect() 
            ->route('pelaksanaan.index', ['plan_id' => $plan->id]) 
    ...
    { 
        ...
        if ($request->has('lahan_id')) { 
            $plan = PreProductionPlan::where('lahan_id', $request->lahan_id) 
                ->where('user_id', Auth::id())
                ->where('is_active', true)
                ->whereColumn('current_day', '<=', 'duration_days') // GEMBOK 2 JUGA DITERAPKAN DI SINI
                ->latest()
                ->first(); 
        ...
        if (!$plan) { 
            return response()->json([ 
                'status' => 'success', 
                'current_day' => 0, 
                'phase_name' => 'Belum Ada Aktivitas', 
                'total_tasks_today' => 0, 
                'tasks' => [], 
                'checked_task_ids' => [], 
                'message' => 'Lahan ini belum memiliki jadwal aktif atau masa tanam telah selesai.' 
            ]); 
        } 

        $checkedTaskIds = \App\Models\ExecutionTaskCheck::where('user_id', Auth::id()) 
            ->where('pre_production_plan_id', $plan->id)
            ->where('day_number', $plan->current_day) 
            ->where('is_done', true)
            ->pluck('planting_guide_task_id'); 
            
        $result = $taskService->generateDailyTasks($plan); 
        $result['checked_task_ids'] = $checkedTaskIds; 
    ...
    { 
        ...
        ExecutionTaskCheck::updateOrCreate( 
            [ 
                'user_id' => Auth::id(), 
                'pre_production_plan_id' => $request->plan_id, 
                'planting_guide_task_id' => $request->task_id, 
                'day_number' => $request->day_number, 
            ], 
            [ 
                'is_done' => $request->is_done, 
                'checked_at' => $request->is_done ? now() : null, 
            ] 
        ); 
        
        if ($request->is_done) { 
            $plan = \App\Models\PreProductionPlan::with(['lahan', 'user', 'commodityType'])->find($request->plan_id); 
            if ($plan) { 
                $daysLeft = (int) $plan->duration_days - (int) $plan->current_day; 
                if ($daysLeft === 7) { 
                    try { 
                        \Illuminate\Support\Facades\Mail::to(Auth::user()->email) 
                            ->send(new \App\Mail\HarvestReminderMail($plan)); 
                    } catch (\Exception $e) { 
                        // Silent fail agar javascript tidak error 
                    } 
                } 
            } 
        } 
    ...
    private function checkAndSendFinancialAlert(PreProductionPlan $plan): void 
    { 
        if ((float) $plan->budget <= 0) { 
            return; 
        } 
        ...
        if ($percentage >= 80) { 
            ...
            if ($plan->user && $plan->user->email) { 
                \Illuminate\Support\Facades\Mail::to($plan->user->email) 
                    ->send(new \App\Mail\FinancialAlertMail($plan, $totalSpent, $percentage)); 
            } 
        } 
    } 
}